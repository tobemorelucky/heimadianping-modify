...
3. 使用 `search_application_logs` 查找 Consumer 停止、订单创建失败、异常或超时的独立证据。
4. 根据多来源 Evidence 决定报告、重新规划或给出不确定结论。

## 业务链路判断

- 请求数量、Lua 准入成功数量和 Kafka 发送数量基本一致，但订单创建成功数量明显下降：异常范围位于 Kafka 发送之后，应优先调查 Consumer，并在未来 MySQL Tool 可用后检查数据库；不能仅凭指标断言 MySQL 是根因。
